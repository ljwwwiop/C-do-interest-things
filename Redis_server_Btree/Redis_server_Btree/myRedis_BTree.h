...
public:
	...
	// 使用Btree的指针数组做为服务端的存储资源方式，类似index，取代了map
	myDB2* DB2[dbNum];
	// 命令集合
	// 用一个map表示存储在服务端的命令集合
	// key为命令的名字，value为一个指向对应的函数
...
class BTreeNode {
	...
	string *strs; // value的类型是string数组
	int t; // min degree(defines the range for number of keys)
...
public:
	...
	// 搜索一个key在sub tree在这个node
	BTreeNode *search(int k);

	// 一个函数用来返回第一个key的index 或者 等于k值的
	int findKey(int k);
...
BTreeNode::BTreeNode(int t1, bool leaf1)
{
	...
	// allocate memory for max number of possible keys
	// and child pointers
	keys = new int[2 * t - 1];
...
void BTreeNode::remove(int k)
{
	...
	if (idx < n &&keys[idx] == k)
	{
		...
		// 如果这个node是一个叶子节点，那么这个key是不在tree中
		if (leaf)
		{
